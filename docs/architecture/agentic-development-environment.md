...
**What is enforced.** The lint command checks ID/file consistency, enums, links, project/cycle/member references, exclusive tag groups, missing blocker/duplicate rationale, ready issues with open blockers, and cycles. It warns about asymmetric `blocked_by`/`blocks` relations. `timestamp` is refreshed by tracker writes, and `index.md` is generated by command.

**Important observed gaps.** Documentation says declare an edge once and have tooling mirror `blocks`; the actual `set`/`move` implementation merely patches requested fields and does not mirror relations. Asymmetry is a warning, not an error. Documentation says start from `next` and only dependency-free specified issues become `ready`; however `next` currently includes unblocked `triage`, `backlog`, and `todo` as well as `ready`. Also, `done` is a direct status shorthand: neither it nor the linter verifies checked acceptance, review evidence, ownership, or human sign-off. The behavioral rules remain valuable social contracts, but another evaluator should not credit them as hard guarantees.
...
- Tracker lifecycle and Definition of Done are mostly norms. The CLI lets an issue move to arbitrary states and `done` without validating acceptance evidence, review, or an approver.
- `next` includes unblocked triage/backlog/todo issues, which conflicts with prose that says start only ready work. An agent following command output rather than the instructions may start unscheduled or under-specified work.
- `blocks` is not automatically mirrored and relationship asymmetry is only warned. The graph can become misleading.
...
- Current tracker smoke testing is dependent on child-process capability; restricted agent sandboxes can report `EPERM` even when direct tracker commands work. The environment should distinguish tool-sandbox failures from application failures consistently.
...
|---|---|---|---|
| 1 | Reconcile tracker behavior with its contract: make `next` return only `ready` by default (or rename/document modes); mirror dependency edges; make asymmetry an error. | High / low–medium / low migration risk. | The executable queue becomes trustworthy for autonomous pickup. |
| 2 | Add state-transition guards: require acceptance checklist/review evidence for `done`; restrict an agent’s normal transition to `in_review`; support explicit approver metadata. | High / medium / medium workflow friction. | Stronger truthful work history and safer autonomy. |
